...
use pinocchio_ata_program::test_utils::{AtaImplementation, AtaVariant};
...
impl CommonTestCaseBuilder {
    ...
    fn build_with_config_and_iteration(
        ...
        if let Some(1) = max_iterations {
            ...
            if matches!(config.base_test, BaseTestType::RecoverNested) {
                    ...
                    .find(|m| matches!(m, SpecialAccountMod::NestedAta { .. }))
                {
                    ...
                    let all_implementations = AtaImplementation::all();
                    let ata_program_ids = [
                    ...
                    .find(|m| matches!(m, SpecialAccountMod::NestedAta { .. }))
                {
                    ...
                    // Update the config with optimal signers and correct threshold
                    if let Some(multisig_mod) = config
                        .special_account_mods
                        .iter_mut()
                        .find(|m| matches!(m, SpecialAccountMod::MultisigWallet { .. }))
                    {
                        if let SpecialAccountMod::MultisigWallet { signers, threshold } =
                            multisig_mod
                        {
                            *signers = optimal_signers.clone();
                            *threshold = 2; // Always use 2-of-N threshold for RecoverMultisig
                        }
                    }
                ...
                // This ensures both P-ATA and SPL ATA use the SAME optimized wallet address
                let all_implementations = AtaImplementation::all();
                let ata_program_ids = [
                    all_implementations.spl_impl.program_id,
                    all_implementations.pata_legacy_impl.program_id,
        ...
        pinocchio_ata_program::debug_log!("🐛 [DEBUG] Built instruction with data: {:?}", ix.data);

        if let Some(failure_mode) = &config.failure_mode {
            pinocchio_ata_program::debug_log!(
                "🐛 [DEBUG] Applying failure mode: {:?}",
                failure_mode
            );
    ...
    fn build_recover_accounts(
        ...
        if matches!(config.base_test, BaseTestType::RecoverMultisig) {
            pinocchio_ata_program::debug_log!("🔍 [DEBUG] RecoverMultisig addresses: wallet={}, token_program={}, owner_mint={}, ata_program={}, owner_ata={}", 
    ...
    fn build_recover_metas(
        ...
        if matches!(config.base_test, BaseTestType::RecoverMultisig) {
            ...
            pinocchio_ata_program::debug_log!("🔍 [DEBUG] RecoverMultisig: {} accounts, signers from idx {}, using 2 of 3 signers", accounts.len(), signer_start);
    ...
    fn build_instruction_data(
        ...
        if config.instruction_discriminator <= 1 {
            use pinocchio_ata_program::test_utils::{
                encode_create_ata_instruction_data, CreateAtaInstructionType,
            };

            // SPL ATA only supports simple discriminator - no bump/rent/token_account_len parameters
            let all_implementations = AtaImplementation::all();
            if ata_implementation.program_id == all_implementations.spl_impl.program_id {
                return vec![config.instruction_discriminator];
            }

            // P-ATA path: supports bump, rent, and token_account_len optimizations
            ...
            pinocchio_ata_program::debug_log!(
                "🐛 [DEBUG] Early return path - encoded data: {:?}",
                data
            );
            data
        } else {
